...
def group_group_collide(missile_group, rock_group):
    ...
    for sprite in missile_group:
        if group_collide(rock_group, sprite):
            missile_removed.add(sprite)
    removed = len(missile_removed)
    missile_group.difference_update(missile_removed)
    return removed
...
def process_sprite_group(sprite_group, canvas):
    ...
    for sprite in sprite_group:
        sprite.draw(canvas)
        if sprite.update():
            sprite_removed.add(sprite)
    sprite_group.difference_update(sprite_removed)
...
def draw(canvas):
    ...
    if lives == 0:
        ...
        for rock in rock_group:
            discard_rocks.add(rock)
        rock_group.difference_update(discard_rocks)
        soundtrack.pause()
